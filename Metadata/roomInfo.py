...
_randomizerSrcFolder = "/home/jstephens/bin/GameModding/"
config = {
	"yamlFolder": os.path.dirname(os.path.realpath(__file__)) + "/",
	"randomizerSourcePath": _randomizerSrcFolder + "RandomizerMod/",
	"itemRandomizerSourcePath": _randomizerSrcFolder + "HollowKnight.ItemChanger/",
}
...
def loadData():
	...
	with open(config["yamlFolder"] + "roomMeta.yaml", "rb") as f:
...
def loadRandomizerData():
	...
	for id in ['rooms', 'transitions', 'locations', 'items']:
	...
	for locId, data in randomizerData['locations'].items():
		...
		try:
			...
			if locId in randomizerData["items"]:
				itemInfo = randomizerData["items"][locId]
				itemInfo["id"] = locId
			else:
				itemInfo = {'id': locId}
	...
	for id, transitionData in randomizerData["transitions"].items():
		roomId = transitionData["SceneName"]
		if roomId not in roomData: continue
		if "transitions" not in roomData[roomId]: roomData[roomId]["transitions"] = {}
		transitionData['id'] = id
		roomData[roomId]["transitions"][id] = transitionData
		# roomData[roomId]["transitions"][transitionData["DoorName"]] = transitionData
